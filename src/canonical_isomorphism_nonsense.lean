-- Kenny's non-canonical proof that something iso to a three term exact seq is exact
-- then my ghastly application coming from localization.
...
-- statement from the ring theory lemma. 