...
import Kenny_comm_alg.Zariski localization tensor_product
...
def lemma_standard_open_1c (R : Type u) [comm_ring R] (f : R) (g : R) (H : Spec.D'(g) ⊆ Spec.D'(f)) :
  localization.loc R (powers f) → localization.loc R (powers g) := sorry

-- I can't do the next part because I am currently not allowed tensor products

-- def lemma_standard_open_1d (R : Type u) [comm_ring R] (f : R) (g : R) (H : Spec.D'(g) ⊆ Spec.D'(f)) (M : module R) :
